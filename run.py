...
@click.command()
@click.option("--num", type=int, default="1", help="neighbors的n值")
@click.option("--train_data", "-td", type=str, default="./data/housing.csv", help="所需数据")
def train(num, training_data):
    ...
    with mlflow.start_run():
        ...
        # 建模训练
        # 在平台中记录当前子运行超参数值
        mlflow.log_param("n", num)

        logging.info("knn n={}".format(num))
        knn_reg = neighbors.KNeighborsRegressor(num)
        knn_reg.fit(train_data, train_value)

        predict_value = knn_reg.predict(test_data)
        show_predict_result(test_data, test_value, predict_value, num)
        ...
        # 保存当前模型文件
        mlflow.sklearn.log_model(sk_model=knn_reg, artifact_path="model_n_{}".format(num), pip_requirements="requirements.txt")
