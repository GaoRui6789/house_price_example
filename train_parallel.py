...
@click.option("--max_n", "-max", type=int, default=7, help="neighbors最大n值")
@click.option("--max_p", type=int, default=2, help="最大并行数量.")
@click.option("--training_data", "-td", type=str, default="./data/housing.csv", help="所需数据")
def train(min_n, max_n, max_p, training_data):

    # show_data_summary(test_set)
    k_range = range(min_n, max_n)

    with ThreadPoolExecutor(max_workers=max_p) as executor:
        _ = [executor.submit(run, n, training_data) for n in k_range]
        # _ = executor.map(run, [n for n in k_range], [train_data], [train_value], [test_data], [test_value])

        for future in as_completed(_):
            print(future.result())
